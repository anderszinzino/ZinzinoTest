...
(function ($) {
    $.fn.extend({
        monthly: function (options) {
            ...
            $('#' + uniqueId).prepend('<div class="monthly-header hide"><div class="monthly-header-title"></div><a href="#" class="monthly-prev"></a><a href="#" class="monthly-next"></a></div>').append('<div class="monthly-event-list"></div>');
            ...
            function setMonthly(m, y) {
                ...
                $('#zzCalendarHeader').data('setMonth', m).data('setYear', y);
                ...
                if (setMonth == currentMonth && setYear == currentYear) {
                    //SETS STYLE TO CURRENT DAY IN CALENDAR
                    //$('#' + uniqueId + ' *[data-number="' + currentDay + '"]').addClass('monthly-today');
                }

                // Reset button
                //alert('(else) setMont: ' + setMonth + ' setYear: ' + setYear);
                if (setMonth == currentMonth && setYear == currentYear) {
                    $('#' + uniqueId + ' .monthly-header-title').html(monthNames[m - 1] + ' ' + y);
                    $('#zzCalendarHeader .monthly-zinzino-title').html(monthNames[m - 1] + ' ' + y);
                } else {
                    //$('#' + uniqueId + ' .monthly-header-title').html(monthNames[m - 1] + ' ' + y + '<a href="#" class="monthly-reset" title="Back To This Month"></a> ');
                    //$('#zzCalendarHeader .monthly-zinzino-title').html(monthNames[m - 1] + ' ' + y + '<a href="#" class="monthly-reset" title="Back To This Month"></a> ');
                    $('#' + uniqueId + ' .monthly-header-title').html(monthNames[m - 1] + ' ' + y);
                    $('#zzCalendarHeader .monthly-zinzino-title').html(monthNames[m - 1] + ' ' + y);
                }
                    ...
                    $.get('' + options.xmlUrl + '', function (d) {
                        $(d).find('event').each(function () {
                            ...
                            if (!fullendDate && startMonth == setMonth && startYear == setYear) {
                                // Add Indicators
                                $('#' + uniqueId + ' *[data-number="' + startDay + '"] .monthly-indicator-wrap').append('<div class="monthly-event-indicator"  data-eventid="' + eventId + '" style="color:' + eventColor + '" title="' + eventTitle + '">' + eventTitle + '</div>');
                                ...
                                for (var i = parseInt(startDay) ; i <= parseInt(endDay) ; i++) {
                                    ...
                                    if (i == parseInt(startDay)) {
                                        $('#' + uniqueId + ' *[data-number="' + i + '"] .monthly-indicator-wrap').append('<div class="monthly-event-indicator" data-eventid="' + eventId + '" style="color:' + eventColor + '" title="' + eventTitle + '">' + eventTitle + '</div>');
                                    } else {
                                        $('#' + uniqueId + ' *[data-number="' + i + '"] .monthly-indicator-wrap').append('<div class="monthly-event-indicator" data-eventid="' + eventId + '" style="color:' + eventColor + '" title="' + eventTitle + '"></div>');
                                    }
                                    multidaylist();
                                }
            ...
            // Advance months
            $(document.body).on('click', '#zzCalendarHeader .monthly-next-zinzino', function (e) {
                var uniqueId = 'mycalendar';
                var setMonth = $('#' + uniqueId).data('setMonth'),
                    setYear = $('#' + uniqueId).data('setYear');
                if (setMonth == 12) {
                    var newMonth = 1,
            ...
            // Go back in months
            $(document.body).on('click', '#zzCalendarHeader .monthly-prev-zinzino', function (e) {
                var uniqueId = 'mycalendar';
                var setMonth = $('#' + uniqueId).data('setMonth'),
                    setYear = $('#' + uniqueId).data('setYear');
                if (setMonth == 1) {
